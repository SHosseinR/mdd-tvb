...
class SpectralFeatureTransformer:
    ...
    def _raw_blocks(self, csd: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        return auto_flat, cross_flat

    def transform_blocks(self, csd: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        auto, cross = self._raw_blocks(csd)
        ...
        return auto_score, cross_score

    def transform(self, csd: np.ndarray) -> np.ndarray:
        auto, cross = self.transform_blocks(csd)
...
def fit_spectral_transformer(
    ...
    auto, cross = shell._raw_blocks(train_csd)
    ...
    if reliability_first is not None and reliability_second is not None:
        ...
        first_auto, first_cross = shell._raw_blocks(reliability_first.csd[train_indices])
        second_auto, second_cross = shell._raw_blocks(reliability_second.csd[train_indices])
